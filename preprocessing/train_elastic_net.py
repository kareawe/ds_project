...
def fit_and_eval(
    ...
    tol: float,
    zscore_clip: float | None,
    clip_log_prediction_to_train_range: bool,
    ...
    if clip_log_prediction_to_train_range:
        y_eval_log_pred = np.clip(y_eval_log_pred, y_train_log_min, y_train_log_max)
...
def main() -> int:
    ...
    clip_log_prediction_to_train_range = not args.disable_log_target_clipping
        ...
        for l1_ratio in args.l1_ratio_grid:
            ...
            for fold_id in sorted(train_df["cv_fold"].unique().tolist()):
                ...
                metrics, _, _, _, _, _ = fit_and_eval(
                    ...
                    tol=args.tol,
                    zscore_clip=zscore_clip,
                    clip_log_prediction_to_train_range=clip_log_prediction_to_train_range,
                )
    ...
    np.savez_compressed(
        ...
        clip_log_prediction_to_train_range=np.asarray(clip_log_prediction_to_train_range, dtype=bool),
    )
