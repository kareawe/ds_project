...
def main() -> int:
    ...
    subset = df[df["source_batch"] == args.train_batch].copy()
    if subset.empty:
        raise RuntimeError(f"No rows found for train batch: {args.train_batch}")
    ...
    default_dir = Path("data/experiments") / f"{sanitize_name(args.train_batch)}_holdout_cv"
    ...
    print(
        "[experiment] "
        f"train_batch={args.train_batch} total={len(subset)} train={len(train_df)} "
